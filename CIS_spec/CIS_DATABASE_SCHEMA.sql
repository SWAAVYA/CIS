...
CREATE TABLE cases (
    ...
    last_briefing_at TIMESTAMPTZ,      -- AMENDED: for LP session definition
    CHECK (status IN ('ACTIVE','CLOSED'))
    -- NOTE: signal_count, hypothesis_count, briefing_count REMOVED (concurrency risk)
    -- Use computed counts via SELECT COUNT() with case_id indexes
);
...
-- AMENDED: audit trail for independence changes
...
CREATE TABLE signals (
    ...
    -- AMENDED: lifecycle_status replaces status (QUARANTINED and ELEVATED removed)
    lifecycle_status    TEXT NOT NULL DEFAULT 'CANDIDATE',
    -- CANDIDATE | ADMITTED | RETAINED | ASSESSED | RESOLVED | ARCHIVED | EXPIRED

    -- AMENDED: governance flags (replace QUARANTINED/ELEVATED states)
    is_quarantined      BOOLEAN NOT NULL DEFAULT FALSE,
    is_connected        BOOLEAN NOT NULL DEFAULT FALSE,
    is_wsp_protected    BOOLEAN NOT NULL DEFAULT FALSE,

    -- AMENDED: SHG tagging for rule-based mode
    ...
    -- SI scoring (4 dimensions + composite)
    ...
    -- NL4-B significance (6 criteria + composite)
    sig_si              NUMERIC(4,3),
    sig_persistence     NUMERIC(4,3),  -- AMENDED: based on signal_event_count, not observation_period
    sig_corroboration   NUMERIC(4,3),
    sig_proximity       NUMERIC(4,3),
    sig_rarity          NUMERIC(4,3),
    sig_relevance       NUMERIC(4,3),  -- AMENDED: capped at 0.20, evidence-referenced hypotheses only
...
-- Signal lifecycle event log (complete audit trail)
-- AMENDED: adds governance_change field for flag toggle events
CREATE TABLE signal_events (
    ...
    governance_change TEXT,            -- AMENDED: "is_quarantined: FALSE → TRUE"
    reason          TEXT NOT NULL,
    lp_flag         TEXT,
    job_run_id      UUID,              -- AMENDED: link to job that caused this event
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Admission audit (mandatory per SLS Section 10)
...
-- AMENDED: Score change audit (new table)
...
CREATE TABLE signal_connections (
    ...
    hypothesis_id           UUID,      -- FK added after hypotheses table created
...
CREATE TABLE contradictions (
    ...
    -- AMENDED: prevent self-contradiction and duplicate pairs
    CHECK (signal_a_id != signal_b_id),
    -- Normalize ordering: enforce signal_a_id < signal_b_id at application level
    -- (UUID comparison; enforce in service layer before insert)
    UNIQUE(signal_a_id, signal_b_id)
);
...
CREATE TABLE released_options (
    ...
    unflag_reason   TEXT,              -- AMENDED: reason for clearing a match flag
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
-- AMENDED: case-insensitive unique text per case
...
CREATE TABLE hypotheses (
    ...
    independence_basis_changed  BOOLEAN NOT NULL DEFAULT FALSE,  -- AMENDED
    resolved_at                 TIMESTAMPTZ,
    resolution_basis            TEXT,
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    -- AMENDED: one hypothesis per connection
    UNIQUE(connection_id)       -- allows multiple NULLs (investigator-created)
);
...
-- FK from signal_connections to hypotheses (added after both tables exist)
ALTER TABLE signal_connections
    ADD CONSTRAINT fk_connection_hypothesis
    FOREIGN KEY (hypothesis_id) REFERENCES hypotheses(id);

-- Evidence
-- AMENDED: UNIQUE on (hypothesis_id, signal_id) prevents double-counting
CREATE TABLE hypothesis_evidence (
    ...
    -- AMENDED: prevent same signal used twice per hypothesis
...
CREATE TABLE plausibility_history (
    ...
    job_run_id      UUID,              -- AMENDED: link to job if background-generated
...
CREATE TABLE briefings (
    ...
    signals_quarantined INT NOT NULL DEFAULT 0,  -- count of is_quarantined = true
    signals_connected   INT NOT NULL DEFAULT 0,  -- count of is_connected = true
