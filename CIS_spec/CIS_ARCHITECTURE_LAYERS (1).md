...
## UI Design Constraints — Structural Resistance to Hypothesis Dominance

**Date added:** 2026-06-02

### The terminology problem

"Signal" in everyday language means alert, warning, indicator. What CIS holds in the observation pool is more specific: an observation being preserved because the current explanatory framework cannot fully account for it. These are not the same thing. An alert demands a response. An observation awaiting explanation demands patience.

The UI must use language that communicates the correct epistemic status:

| Technical term | UI term | Reason |
|----------------|---------|--------|
| Signal | Observation | A record of something seen — neutral, precise |
| Signal Pool | Open Observations | Conveys incompleteness — these are still open |
| High-significance signals | Unexplained observations (high structural weight) | Makes unexplained status explicit |
| Open Questions | Uncovered observations | Not covered by any hypothesis — precise |

"Signal" remains the internal technical term in the database, services, and API. "Observation" is surfaced in the UI. The architecture document uses both. The UI uses the one that communicates the right epistemic status to an investigator who has not read the spec.

---

### The hypothesis dominance risk

The natural dashboard hierarchy — observations → hypotheses → briefing — trains investigators to treat hypothesis generation as the goal. An investigator who reaches hypotheses quickly and finds them plausible will start reading every subsequent observation through the hypothesis lens. This recreates the compression dynamic CIS exists to resist.

This is Epistemic Capture at the intake level, operating before the reasoning layer and before PDI would detect it. The investigator is not processing observations independently — they are looking for observations that support the existing hypothesis. The pool becomes a confirmation pool. The residuals are never formed.

The UI must structurally resist this. Not through warnings or instructions — those are ignored — but through emphasis and hierarchy.

---

### Design constraints

**Constraint 1 — Unexplained observations are the primary metric.**

The numerically prominent figure on the dashboard is not the hypothesis count. It is the count of uncovered observations — observations not accounted for by any active hypothesis. Hypotheses are secondary.

Correct dashboard header:
```
14 observations     9 unexplained     3 hypotheses     0 quarantined
```

Not: "3 hypotheses, 14 signals." The number that should create productive discomfort is uncovered observations. The hypothesis count creates satisfaction. Satisfaction is the enemy of residual preservation.

**Constraint 2 — Residual view is the default in the observation pool.**

The observation pool's default filter is UNEXPLAINED — observations with no hypothesis coverage. Not ALL. Investigators see uncovered observations first, hypothesis-covered observations second. This is a structural choice about what investigators habitually attend to.

**Constraint 3 — Hypothesis sections display coverage explicitly.**

The hypothesis section does not show "3 active hypotheses." It shows:

```
3 hypotheses — covering 5 of 14 observations
9 observations remain uncovered
```

The nine remaining uncovered is what matters. It is the measure of how much the investigation still does not explain. The hypothesis count measures explanatory progress. The uncovered count measures explanatory incompleteness. CIS is optimized for the latter.

**Constraint 4 — Hypothesis generation does not change the observation pool's visual treatment.**

When SHG generates a hypothesis and two observations become "covered," those observations do not disappear from the pool view or become visually subordinate. They are still present, still visible, still assessable independently. Coverage by a hypothesis is a relationship, not a reclassification. An observation covered by a hypothesis that is later falsified must return to uncovered status without any sense that it is being "reopened" — it was always there.

---

### The design principle

The UI should structurally resist the investigator's natural desire for explanatory closure. Every UX instinct pulls toward making hypotheses feel like progress. Hypotheses are satisfying. Uncovered observations are uncomfortable. CIS must be designed against that grain.

An observation that remains unexplained after thorough investigation is more valuable to the investigation than an observation that was quickly absorbed into a hypothesis. The UI must enforce this valuation structurally — through what is numerically prominent, what is shown by default, and how coverage is displayed — not through warnings or instructions that will be ignored.

This is not a UX preference. It is an architectural constraint that follows directly from the system's objective function: preserve and connect unexplained structure rather than minimize it.

---

*UI Design Constraints | 2026-06-02*
